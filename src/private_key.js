...
const asyncPromise = require('./async-promise')
...
function PrivateKey(d) {
    ...
    } else if(typeof d === 'object' && bigi.isBigInteger(d.d)) {
...
function unitTest() {
  ...
  assert.equal(pubkey, 'XMX859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM',
    'pubkey string comparison test failed on a known public key')

    throwWrapper(() => PrivateKey.fromWif(wif), 'converting known wif from string')
    throwWrapper(() => PublicKey.fromString(pubkey), 'converting known public key from string')

  unitTested = true
}

/** @private */
const throwWrapper = (cb, msg) => {
  try {
    cb()
  } catch(error) {
